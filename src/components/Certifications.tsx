...
const CertificationCard: React.FC<{ cert: Certification; isExpired?: boolean }> = ({ cert, isExpired = false }) => (
  ...
  >
    <div className="flex items-start justify-between mb-4">
      ...
      {cert.credentialId && (
        <a
          href={`https://learn.microsoft.com/api/credentials/share/en-us/${cert.credentialId}?tmpl=certification`}
          target="_blank"
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {activeCertifications.map((cert) => (
              <CertificationCard key={cert.credentialId ?? cert.name} cert={cert} />
            ))}
          </div>
        </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {expiredCertifications.map((cert) => (
                <CertificationCard key={cert.credentialId ?? cert.name} cert={cert} isExpired={true} />
              ))}
            </div>
          </div>
        )}
